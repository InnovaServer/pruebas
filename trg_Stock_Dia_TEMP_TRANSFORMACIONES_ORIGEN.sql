...
ALTER TRIGGER trg_Stock_Dia_TEMP_TRANSFORMACIONES_ORIGEN ON TEMP_TRANSFORMACIONES_ORIGEN
...
begin
	...
	begin
		...
		if @Inventariado_New <> @Inventariado_Old and @Inventariado_New > 0 -- Se está inventariando
			begin
				EXEC sp_Stock_Dia_Insert @IdArticulo,@Fecha,@lote,'',@Ubicacion,@Almacen,@Cantidad,'Transformaciones_Origen'
			end
	...
	begin
		...
		if @Inventariado_New <> @Inventariado_Old and (@Inventariado_Old > 0) -- Se está desinventariando
			begin
				...
				EXEC sp_Stock_Dia_Insert @IdArticulo,@Fecha,@lote,'',@Ubicacion,@Almacen,@Cantidad_Negativa,'Transformaciones_Origen'
			end
