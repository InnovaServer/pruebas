...
GO
ALTER FUNCTION [dbo].[fn_STOCK_Envio_Ordenes_Confeccion_DESTINO]
(
    @IdArticulo int,
	@HastaFecha date
)
...
---------------------------------------------------------------------------------------------------------------------------------------
-- Primero se define una tabla de trabajo que contiene todas las órdenes de confección entre la fecha del ultimo inventario y la fecha a la que se pide el stock
...
select * from fn_STOCK_Movtos_Envio_Ordenes_Confeccion_DESTINO(@IdArticulo,'01/01/1900',@HastaFecha,0) -- Se pasa el parámetro como si no fuera para stock para que tenga en cuenta la fecha desde y coja todas las órdenes de confección
---------------------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------------------
...
declare iterador cursor for 
	...
	select 
		...
		DISPOSICIONES.PROVEEDOR as Proveedor,
		DISPOSICIONES.FECHA as FechaOrden
...
open iterador
fetch next from iterador into @IdArticulo_Produccion,@Cantidad_Produccion,@Almacen_MateriasPrimas,@Proveedor,@FechaOrden
while @@fetch_status = 0
	begin
		...
		while @@fetch_status = 0
			begin
				-- Solo se graba en stock si la fecha del ultimo inventario del artículo del escandallo es anterior a la de la orden
				if @FechaOrden > = dbo.fn_Fecha_Ultimo_Inventario (@HastaFecha,@Articulo_escandallo,'','',-1,@Almacen_Proveedor)
					begin
						...
						insert into @Consumos (IdArticulo,Lote,Pieza,Ubicacion,Almacen,Cantidad)
							values (@Articulo_escandallo,'','',-1,@Almacen_Proveedor,@Cantidad_consumida)
					end
